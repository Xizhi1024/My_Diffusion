...
def _denormalise_pet_np(pet_norm: np.ndarray, meta: dict) -> np.ndarray:
    ...
    if meta.get("suv_ok", False):
        suv_max = float(meta.get("pet_suv_max", 20.0))
        return p01 * suv_max
    else:
        v_min = float(meta.get("pet_raw_min", 0.0))
        v_max = float(meta.get("pet_raw_max", 1.0))
        return p01 * max(v_max - v_min, 1e-8) + v_min
...
def evaluate(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        for i in range(B):
            ...
            # Clinical SUV metrics (only meaningful if lesion mask exists)
            if mask_np.sum() > 0:
                suv_m = compute_suv_metrics(pred_np, target_np, mask_np, organ_np, meta)
...
def main():
    ...
    if data_cfg.get("use_fake_data", False):
        ...
        try:
            ...
            print(f"[evaluate] No {args.split} samples in {cache_dir}, trying train split...")
