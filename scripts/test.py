...
from src.dataset import PairedDataset, get_dataloaders
...
def save_comparison_images(ct, pet, generated_pet, save_dir, idx, suffix=''):
    """Save CT condition, GT PET, generated PET side by side."""
    ct = np.clip((ct + 1.0) / 2.0, 0.0, 1.0)
    pet = np.clip((pet + 1.0) / 2.0, 0.0, 1.0)
...
def test_model(inference_model, test_loader, num_inference_steps, num_samples=None, save_dir=None):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            target_01 = torch.clamp((pet + 1.0) * 0.5, 0.0, 1.0)
            pred_01 = torch.clamp(generated, 0.0, 1.0)
