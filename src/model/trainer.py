...
def _build_optimizer(model, training_cfg: Dict[str, Any]) -> torch.optim.AdamW:
    ...
    if not bool(group_cfg.get("enabled", False)):
        # Only trainable parameters may enter the optimizer.  Frozen modules
        # (e.g. the perceptual-x0 encoder, which has requires_grad=False params
        # owned by a loss term) must never receive optimizer state.
        trainable = [p for p in model.parameters() if p.requires_grad]
        return _fused_adamw(trainable, lr=base_lr, wd=base_wd)
