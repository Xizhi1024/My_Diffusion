...
class TinySegmenter(nn.Module):
    ...
    def forward(self, pet: torch.Tensor) -> torch.Tensor:
        ...
        if self.stage == 1:
            ...
            # Stage 2 (default): signed-distance-like continuous output.
            # Linear (no tanh): tanh saturates at ±1, so a model that drifts
            # toward all-background output (-1) loses its gradient entirely and
            # never recovers — the observed recall-0 failure (loss pinned ~0.563
            # over all 20 Stage-2 epochs).  Linear output keeps lesion-region
            # gradient alive regardless of sign/magnitude.  L1-based losses
            # (segmenter_loss, SegmenterConsistencyLoss) do not depend on a
            # bounded range, so the change is safe for both.
            return raw
...
def _pseudo_distance_transform(mask: torch.Tensor) -> torch.Tensor:
    ...
    The normalisation scale is the maximum distance *inside the lesion*
    (``d_in.max()``), NOT the global max over the whole image.  A global max is
    dominated by background pixels far from any lesion (tens to ~100 px), which
    crushes a small lesion's already-tiny interior signal to ~0.01 and leaves
    the model with no learnable gradient — Stage-2 loss pinned ~0.563, recall 0.
    Anchoring to the lesion interior gives the lesion centre a strong signal
    (~0.76) while background still saturates near -1.
    """
    try:
        import numpy as np
    ...
    for idx in np.ndindex(mask_np.shape[:2]):
        m = mask_np[idx]
        if not m.any():
            out[idx] = np.zeros_like(m, dtype=np.float32)
            continue
        d_in = distance_transform_edt(m).astype(np.float32)
        d_out = distance_transform_edt(~m).astype(np.float32)
        signed = d_in - d_out
        # Anchor the scale to the lesion interior, not the far background.
        scale = max(float(d_in.max()), 1.0)
        out[idx] = np.tanh(signed / scale).astype(np.float32)
...
def segmenter_loss(
    ...
    Args:
        ...
        lesion_weight: extra weight on lesion pixels for stage-2 relaxed L1.
            A tiny lesion is ~0.2% of the image; a plain global-mean L1 lets
            the model satisfy the loss by outputting all -1 (background), which
            is exactly the recall-0 failure.  Weighting the lesion region makes
            the model learn to push it positive.

    Returns:
        (loss, logs)
    """
    if stage <= 2:
        if mask is None or stage == 1:
            # Stage 1 heatmap is already ~0.02 background; plain L1 is fine.
            l1 = F.l1_loss(pred, target)
            logs = {"seg/l1": l1.detach()}
            return l1, logs
        # Stage 2: lesion-weighted L1 so the tiny lesion region is not drowned
        # out by the large background.
        mask_b = (mask > 0.5).to(dtype=pred.dtype)
        weight = torch.where(mask_b > 0.5, torch.full_like(mask_b, lesion_weight), torch.ones_like(mask_b))
        diff = (pred - target).abs()
        l1 = (diff * weight).sum() / weight.sum()
        logs = {"seg/l1": l1.detach()}
        return l1, logs
    else:
