...
def validate_png_baseline_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # A segmenter is only usable when it carries a real pretrained checkpoint.
    # The perceptual-x0 P1_SEG_OUT arm enables segmenter_consistency on PNG
    # data; that is legitimate once a frozen checkpoint path is configured.
    segmenter_cfg = config.get("segmenter") or model_cfg.get("segmenter") or {}
    segmenter_ckpt = str(
        (segmenter_cfg.get("checkpoint") or "") if isinstance(segmenter_cfg, dict) else ""
    ).strip()
    segmenter_has_checkpoint = bool(segmenter_ckpt)
    ...
    if status["segmenter_enabled"] and not segmenter_has_checkpoint:
        offenders.append(
            "segmenter is enabled but requires a pretrained PET segmenter "
            "checkpoint; not part of the PNG baseline. "
            "Configure segmenter.checkpoint before enabling it."
        )
    if offenders:
