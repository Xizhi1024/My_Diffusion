"""Standalone training utilities for conditional PET mean predictors."""
...
class MeanPretrainer:
    """Train and select one conditional PET mean predictor."""
    ...
    def __init__(
        ...
        if (
            self.pathology_exclusion_enabled
            and self.mean_architecture in {"full_image_unet", "unet"}
        ):
            raise ValueError(
                "full_image_unet is the strong U-Net anchor and must be trained on "
                "the full target; disable conditional_mean.pathology_exclusion"
            )
    ...
    def _batch_loss(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        with torch.amp.autocast(
            ...
            output = self.predictor(ct)
            if self.mean_architecture in {"full_image_unet", "unet"}:
                return mean_charbonnier_loss(
                    output["mean_pet"],
                    pet,
                    self.epsilon,
                )
