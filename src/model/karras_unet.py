"""
Karras Unet - 幅度保持 U-Net 架构
基于论文: https://arxiv.org/abs/2312.02696 by Karras et al.

这是与原 Diffusion/diffusion 项目中相同的 KarrasUnet 实现
"""
...
class Attention(Module):
    ...
    def __init__(
        ...
        # 注意：这里需要从 attend.py 导入 Attend
        # 如果没有该文件，可以使用标准的注意力机制
        try:
            from ...denoising_diffusion_pytorch.attend import Attend
            self.attend = Attend(flash = flash)
        except ImportError:
            # 简单的 fallback - 使用标准的多头注意力
            self.attend = None
            self.use_standard_attn = True
    ...
    def forward(self, x):
        ...
        if self.attend is not None:
            ...
            # Fallback: 使用 scaled dot-product attention
            scale = self.heads ** -0.5
            attn = torch.softmax(torch.einsum('bhnd,bhmd->bhnm', q, k) * scale, dim=-1)
...
class KarrasUnet(Module):
    ...
    参考论文: https://arxiv.org/abs/2312.02696
    Figure 21. config G

    与原 Diffusion/diffusion 项目中的 KarrasUnet 完全相同
    """
...
# 用于兼容现有代码的包装类
class KarrasUnetWrapper(Module):
    """
    KarrasUnet 的包装类，使其兼容现有的 LatentDiffusionModel 接口

    这个类将 KarrasUnet 包装成与 diffusers.UNet2DModel 类似的接口

    注意：KarrasUnet 的输入/输出通道数相同，因此当 in_channels != out_channels 时，
    我们需要添加输出投影层
    """
    def __init__(
        self,
        ...
        **karras_unet_kwargs
        ...
        # KarrasUnet 的输入/输出通道数相同
        # 我们使用 in_channels 作为 KarrasUnet 的通道数
        self.karras_unet = KarrasUnet(
            image_size=image_size,
            channels=in_channels,
            **karras_unet_kwargs
        )

        # 【修复】如果输出通道数与输入不同，添加输出投影层
    ...
    def forward(self, x, timesteps):
        """
        前向传播，返回带有 .sample 属性的结果对象
        """
        # 【关键修复】将整数时间步 [0, 999] 归一化为连续时间 [0, 1]
        # KarrasUnet 的 MPFourierEmbedding 期望时间范围 [0, 1]
        # 如果 timesteps 是整数类型且值 > 1，则进行归一化
        if timesteps.dtype in (torch.int32, torch.int64, torch.long) or timesteps.max() > 1.0:
            time_continuous = timesteps.float() / 999.0
        else:
            time_continuous = timesteps
        
        # KarrasUnet 输出通道数 = in_channels
        output = self.karras_unet(x, time_continuous)

        # 【修复】投影到目标输出通道数
        output = self.output_proj(output)

        # 为了兼容 diffusers 接口，返回一个带有 .sample 属性的对象
        class OutputWrapper:
...
if __name__ == '__main__':
    ...
    print("KarrasUnet test passed!")

    # 包装类示例 - 用于条件生成 (CT -> PET)
    wrapper_unet = KarrasUnetWrapper(
        image_size = 128,
        in_channels = 2,  # noisy PET + CT condition
        out_channels = 1,
        dim = 128,
        dim_max = 512,
        num_downsamples = 3,
    ...
    # 输入: [batch, 2, H, W] - 通道0是 noisy PET，通道1是 CT condition
    input_tensor = torch.randn(2, 2, 128, 128)
    timesteps = torch.randint(0, 1000, (2,))

    output = wrapper_unet(input_tensor, timesteps)
    print(f"Wrapper output shape: {output.sample.shape}")
    assert output.sample.shape == (2, 1, 128, 128)
    print("KarrasUnetWrapper test passed!")
