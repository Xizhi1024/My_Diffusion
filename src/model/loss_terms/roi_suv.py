...
class ROISUVLoss(LossTerm):
    ...
    def forward(self, ctx: LossContext) -> tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        # TBR (a ratio) excluded from the loss: when the model's predicted
        # background SUV→0 the denominator clamps to 1e-6 and pred_tbr→∞,
        # which dominated training (~99% of total loss) and pushed the model
        # to output all-zeros. SUVmax + SUVmean are sufficient to drive the
        # model to generate lesion uptake. tbr_per is still logged for eval.
        per_sample_loss = suv_max_per + suv_mean_per
