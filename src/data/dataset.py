...
class CachedDataset(Dataset):
    ...
    def __init__(self, cache_dir: str | Path, split: str = "train", *, augment: bool = False,
        ...
        # A manifest path that was explicitly provided but does not exist is a
        # hard error: silently falling back to per-sample _meta.json splits
        # (the old behaviour) could mix training and held-out validation
        # patients and invalidate the whole experiment.
        manifest = None
        if split_manifest is not None:
            manifest_path = Path(split_manifest)
            if not manifest_path.is_file():
                raise FileNotFoundError(
                    f"split_manifest provided but not found: {manifest_path}. "
                    "Refusing to fall back to _meta.json splits because that "
                    "could silently mix train/validation patients."
                )
