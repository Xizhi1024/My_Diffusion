...
Supports:
  ...
  - Direct PNG datasets laid out as root/{train,val}/{ct,pet,label}
...
def _read_png_unit(path: str | Path) -> np.ndarray:
    """Read a grayscale PNG-like image as float32 in [0, 1]."""
    path = Path(path)
    img = Image.open(path)
    raw = np.asarray(img)
    if raw.ndim == 3:
        raw = np.asarray(img.convert("L"))
    arr = _ensure_2d(raw).astype(np.float32)
    if arr.size == 0:
...
def _resize_to_tensor(arr: np.ndarray, size: int, *, mode: str = "bilinear") -> torch.Tensor:
    ...
    if mode == "nearest":
        ...
        t = F.interpolate(t, size=(size, size), mode=mode, align_corners=False)
    return t.squeeze(0)
...
class PNGSliceDataset(Dataset):
    """Direct PNG dataset for root/{train,val}/{ct,pet,label}.

    CT/PET/label images are paired by filename stem. CT can be 512x512 while
    raw PET can be a different size; all tensors are resized to ``image_size``
    for the model, and original PET size is kept in ``meta`` for exporting
    predictions back at PET resolution.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if require_label and (label_root is None or not label_root.is_dir()):
            raise FileNotFoundError(f"Label PNG directory not found: {label_root}")

        ct_files = self._scan_images(ct_root)
        pet_files = self._scan_images(pet_root)
        label_files = self._scan_images(label_root) if label_root and label_root.is_dir() else {}
        ...
        if not sample_ids:
            raise ValueError(
                f"No paired PNG samples for split={split!r} under {self.root}. "
                "Expected matching filenames in ct/, pet/, and label/."
            )

        self.entries: List[PNGEntry] = []
        for sid in sample_ids:
            pid, slc = _parse_sample_id(sid)
            self.entries.append(PNGEntry(
                sample_id=sid,
                patient_id=pid or sid,
                slice_id=slc or 0,
                split=split,
                ct_path=ct_files[sid],
                pet_path=pet_files[sid],
                label_path=label_files.get(sid),
            ))
        self._aug_fn = T.RandomHorizontalFlip(p=0.5) if augment else None
        patient_count = len({e.patient_id for e in self.entries})
        print(
            f"[PNGSliceDataset] split={split!r} samples={len(self.entries)} "
            f"patients={patient_count} image_size={self.image_size} pet_dir={pet_dir!r}"
        )

    @staticmethod
    def _scan_images(root: Optional[Path]) -> Dict[str, Path]:
        ...
        for p in sorted(root.iterdir()):
            ...
            sid = p.stem
            if sid in mapping:
                raise ValueError(f"Duplicate sample_id {sid!r} in {root}")
            mapping[sid] = p
        return mapping
...
def build_dataloaders(data_cfg: Dict[str, Any], run_cfg: Dict[str, Any]) -> Tuple[DataLoader, Optional[DataLoader]]:
    ...
    png_ct_dir = data_cfg.get("png_ct_dir", "ct")
    png_pet_dir = data_cfg.get("png_pet_dir", "pet")
    png_label_dir = data_cfg.get("png_label_dir", "label")
    png_require_label = data_cfg.get("png_require_label", True)
    ...
    if png_root:
        ...
        if not png_root_path.is_dir():
            raise RuntimeError(f"PNG root not found: '{png_root}'")
        train_ds = PNGSliceDataset(
            ...
            ct_dir=png_ct_dir,
            pet_dir=png_pet_dir,
            label_dir=png_label_dir,
            augment=augment,
            require_label=png_require_label,
        )
        val_ds = None
        val_split_name = data_cfg.get("val_split_name", "val")
        if (png_root_path / val_split_name).is_dir():
            try:
                val_ds = PNGSliceDataset(
                    png_root_path,
                ...
                print(f"[DataLoader] No PNG val samples in {png_root_path / val_split_name}")
        ...
        raise RuntimeError(
            f"No usable data source. cache_dir='{cache_dir}', png_root='{png_root}'. "
            "Set data.png_root for PNG folders, data.cache_dir for .npz cache, "
            "or data.use_fake_data: true for smoke testing."
        )
