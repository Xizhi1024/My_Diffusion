...
def _meta_int(meta: dict, key: str, default: int) -> int:
    value = _meta_scalar(meta.get(key), default)
    try:
        return int(value)
...
def _save_prediction_png(pred: np.ndarray, meta: dict, out_dir: Path, batch_idx: int, sample_idx: int) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    arr = np.squeeze(pred)
    arr01 = np.clip((arr + 1.0) / 2.0, 0.0, 1.0)
    img = Image.fromarray((arr01 * 255.0).round().astype(np.uint8), mode="L")
    target_h = _meta_int(meta, "pet_original_h", img.height)
    target_w = _meta_int(meta, "pet_original_w", img.width)
    if target_h > 0 and target_w > 0 and (img.height, img.width) != (target_h, target_w):
        resampling = getattr(Image, "Resampling", Image).BILINEAR
        img = img.resize((target_w, target_h), resampling)
    sample_id = str(_meta_scalar(meta.get("sample_id"), f"{batch_idx}_{sample_idx}"))
    img.save(out_dir / f"{sample_id}_pred_pet.png")
...
def evaluate(
    ...
    if save_samples is not None:
        save_samples.mkdir(parents=True, exist_ok=True)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        for i in range(B):
            ...
            if save_samples is not None:
                _save_prediction_png(pred_np, meta, save_samples, batch_idx, i)
...
def main():
    ...
    ap.add_argument("--save-samples", type=str, default=None, help="Directory to save predicted PET PNGs")
    ...
    from src.data.dataset import CachedDataset, FakeDataset, PNGSliceDataset
    ...
    elif data_cfg.get("png_root"):
        png_root = Path(data_cfg["png_root"])
        split_name = data_cfg.get(f"{args.split}_split_name", args.split)
        try:
            ds = PNGSliceDataset(
                png_root,
                split=split_name,
                ...
                pet_dir=data_cfg.get("png_pet_dir", "pet"),
                label_dir=data_cfg.get("png_label_dir", "label"),
                augment=False,
                require_label=data_cfg.get("png_require_label", True),
            )
        except (FileNotFoundError, ValueError):
            if not args.allow_train_fallback:
                raise RuntimeError(
                    f"No {args.split!r} PNG samples found under {png_root}. "
                    "Pass --allow-train-fallback only for debugging."
                )
            print(f"[evaluate] No {args.split} PNG samples under {png_root}, falling back to train split...")
            ds = PNGSliceDataset(
                png_root,
    ...
    summary = evaluate(
        ...
        device=device,
        amp=not args.no_amp,
        save_samples=Path(args.save_samples) if args.save_samples else None,
    )
