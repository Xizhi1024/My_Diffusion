...
class TestDataset:
    ...
    def test_png_slice_dataset_resizes_and_keeps_pet_shape(self, tmp_path):
        from PIL import Image
        from src.data.dataset import PNGSliceDataset

        for split in ("train",):
            for folder in ("ct", "pet", "label"):
                (tmp_path / split / folder).mkdir(parents=True)
        sid = "001002"
        Image.fromarray(np.full((512, 512), 128, dtype=np.uint8)).save(tmp_path / "train" / "ct" / f"{sid}.png")
        Image.fromarray(np.full((192, 192), 64, dtype=np.uint8)).save(tmp_path / "train" / "pet" / f"{sid}.png")
        label = np.zeros((512, 512), dtype=np.uint8)
        label[120:180, 120:180] = 255
        Image.fromarray(label).save(tmp_path / "train" / "label" / f"{sid}.png")

        ds = PNGSliceDataset(tmp_path, split="train", image_size=256)
        sample = ds[0]
        assert sample["ct"].shape == (1, 256, 256)
        assert sample["pet"].shape == (1, 256, 256)
        assert sample["mask"].shape == (1, 256, 256)
        assert sample["meta"]["pet_original_h"] == 192
        assert sample["meta"]["pet_original_w"] == 192

    def test_build_dataloaders_png(self, tmp_path):
        from PIL import Image
        from src.data.dataset import build_dataloaders
        ...
        for split in ("train", "val"):
            ...
            for idx in range(2):
                sid = f"00100{idx + 1}"
                Image.fromarray(np.full((512, 512), 128, dtype=np.uint8)).save(tmp_path / split / "ct" / f"{sid}.png")
                Image.fromarray(np.full((192, 192), 64, dtype=np.uint8)).save(tmp_path / split / "pet" / f"{sid}.png")
                Image.fromarray(np.zeros((512, 512), dtype=np.uint8)).save(tmp_path / split / "label" / f"{sid}.png")

        data_cfg = {"png_root": str(tmp_path), "image_size": 64, "batch_size": 2, "val_batch_size": 2, "augment": False}
        run_cfg = {"num_workers": 0, "pin_memory": False, "persistent_workers": False}
        train, val = build_dataloaders(data_cfg, run_cfg)
        batch = next(iter(train))
        assert batch["ct"].shape == (2, 1, 64, 64)
