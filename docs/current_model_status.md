...
>
> 基准日期:2026-07-23。生产配置:`configs/experiments/slmf_png_spectral_router_v5.yaml`。
...
| 冻结框架要求 | 代码现状 | 已启用 | 已验证 |
|---|---|---|---|
| `r₀ = y − μ_bg(c)` pathology-**excluded** mean | conditional_mean 冻结;**但生产用的是 included mean(见 §3.4)** | ✅(included) | H2 PASS 的是 **excluded**;生产用的不是 |
...
**半边 B 小结**:病灶保留目前靠 ROI/TopK/peak **损失**兜底;H2 验证的 excluded residual
构造**没进生产**(§3.4)。课程是手设两段 τ 门,不是 H3 驱动的 continuation/homotopy。
...
4. **生产 mean 是 pathology-included(已坐实)**:见下表,生产 hash 与 H2 excluded/included
   均不同,且 MeanPretrainer 代码无 mask 路径、在全 split 上训练。
