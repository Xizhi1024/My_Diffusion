...
class TestResidualBBDMIntegration:
    ...
    def test_full_image_unet_mean_loads_prefixed_comparison_checkpoint(self, tmp_path):
        from src.model.mean_predictor import FullImagePETPredictor
        from src.model.slmf_bbdm import SLMFBBDM

        source = FullImagePETPredictor(base_channels=8)
        checkpoint_path = tmp_path / "unet_comparison.pt"
        torch.save(
            {
                "model": {
                    **{
                    ...
                    "discriminator.placeholder": torch.tensor(0.0),
                },
            },
            checkpoint_path,
        )
        ...
        model = SLMFBBDM.from_config(cfg)
        assert type(model.mean_predictor).__name__ == "FullImagePETPredictor"
        for key, value in source.state_dict().items():
            torch.testing.assert_close(model.mean_predictor.state_dict()[key], value)
    ...
    def test_zero_residual_output_scale_is_exact_mean_fallback(self):
        from src.model.slmf_bbdm import SLMFBBDM

        cfg = _residual_config(frequency=False, gabor=False)
        cfg["modules"]["residual_bridge"]["output_scale"] = 0.0
        model = SLMFBBDM.from_config(cfg)
        result = model.sample(_model_batch(), num_steps=2)

        torch.testing.assert_close(result["synthetic_pet"], result["mean_pet"])
        assert torch.count_nonzero(result["pred_residual"]).item() == 0
        assert torch.count_nonzero(result["raw_pred_residual"]).item() > 0
