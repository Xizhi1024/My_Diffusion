...
from src.model.mean_predictor import FullImagePETPredictor, LowFrequencyPETPredictor
...
def test_full_image_mean_matches_2d_unet_checkpoint_contract():
    from wuzhe.comparison_experiments.models import UNetGenerator

    torch.manual_seed(123)
    baseline = UNetGenerator(base_channels=8)
    predictor = FullImagePETPredictor(base_channels=8)
    predictor.load_state_dict(baseline.state_dict(), strict=True)
    baseline.eval()
    predictor.eval()

    ct = torch.randn(2, 1, 32, 32)
    expected = baseline(ct)
    output = predictor(ct)
    torch.testing.assert_close(output["mean_pet"], expected)
    assert output["ll2"].shape == (2, 1, 8, 8)


def test_full_image_mean_pretrainer_optimizes_pixel_space(tmp_path):
    from src.model.mean_pretraining import MeanPretrainer

    config = _config()
    config["modules"]["conditional_mean"]["architecture"] = "full_image_unet"
    predictor = FullImagePETPredictor(base_channels=8)
